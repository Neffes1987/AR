        ...
        for (key in charList){
            charMap.push(
                <tr key={key+':TR'}>
                    <td>{key}</td>
                    <td><input  type='text'
                                value={charList[key].name}
                                data-id = {key}
                                onChange={(e)=>{
                                    const keyID = $(e.target).attr('data-id');
                                    charInputChange(['charList',keyID,'name'],e.target.value)
                                }}
                        />
                    </td>
                    <td><input  type='number'
                                value={charList[key].value}
                                data-id = {key}
                                onChange={(e)=>{
                                    const keyID = $(e.target).attr('data-id');
                                    charInputChange(['charList',keyID,'value'],e.target.value)
                            }}/>
                    </td>
                    <td><textarea data-id = {key}
                            onChange={(e)=>{
                                const keyID = $(e.target).attr('data-id');
                                charInputChange(['charList',keyID,'description'],e.target.value)
                            }}>
                            {charList[key].description}
                        </textarea>
                    </td>
                    <td>
                        <input  type='button'
                                value='Удалить'
                                onClick={charGChange.bind(null,['charList'],'delete',key)}
                        />
                    </td>
                </tr>
            );
