...
export default React.createClass({
    ...
    componentWillMount() {
        const id = this.props.editItem?this.props.editItem:1;
        this.props.getRemoteData('REMOTE_BD_USER',id);
    },
    render(){
        ...
        for(let key in gameChar){
            charTab.push(<div className='col-md-4'  key={key+'char'}><ItemGroup label={charList[key]?charList[key].name:''} func={DBInputChange.bind(null,['DBList',editItem,'gameChar',key])} type='number' iValue={gameChar[key]}/></div>)
        }

        for(let key in gameLut){
            lutTab.push(<div className='col-md-4'  key={key+'lut'}><ItemGroup label={lutList[key]?lutList[key].label:''} func={DBInputChange.bind(null,['DBList',editItem,'gameLut',key])} type='number'  iValue={gameLut[key]}/></div>)
        }
        return(
